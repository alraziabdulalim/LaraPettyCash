...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Old Transactions :: Update Form') }}
        </h2>
    ...
    <div class="p-10">
        <div class="max-w-2xl mx-auto px-6 py-2 sm:px-2 lg:px-4">
            ...
            <div class="p-5 bg-neutral-700 rounded-lg">
                <form method="POST" action="{{ route('oldVouchers.update', $transaction->id) }}">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mt-3">
                        ...
                        <x-text-input id="voucher_at" class="block mt-1 w-full" type="text" name="voucher_at" :value="$transaction->voucher_at" required autofocus autocomplete="voucher_at" />
                    ...
                    <div class="mt-3">
                        ...
                        <select id="oldactype_id" name="oldactype_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            <option value="{{$oldacType->id}}" {{ $transaction->oldactype_id == $oldacType->id ? 'selected' : '' }}>{{ $oldacType->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-3">
                        ...
                        <select id="parent_id" name="parent_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            <option value="{{$parentAc->id}}" {{ $transaction->parent_id == $parentAc->id ? 'selected' : '' }}>{{ $parentAc->id }}{{ $parentAc->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-3">
                        ...
                        <select id="oldacname_id" name="oldacname_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            <option value="{{$oldacName->id}}" {{ $transaction->oldacname_id == $oldacName->id ? 'selected' : '' }}>{{ $oldacName->id }}{{ $oldacName->s_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-3">
                        ...
                        <x-text-input id="amount" class="block mt-1 w-full" type="text" name="amount" :value="$transaction->amount" required autofocus autocomplete="amount" />
                    ...
                    <div class="mt-3">
                        ...
                        <textarea id="details" name="details" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required autofocus autocomplete="details">{{ $transaction->details }}</textarea>
